...
class Menu extends Phaser.Scene {
  ...
  hideRegisterElement() {
    ...
    this.scene.scene.tweens.add({
      targets: this.registerElement.rotate3d,
      x: 1,
      w: 90,
      duration: 2000,
      ease: "Power3",
  ...
  showUserInfo() {
    if (this.user !== null) {
      ...
      this.userInfoElement.on("click", function (event) {
        if (event.target.id === "Rlogout") {
          Firebase.auth().signOut();
          self.scene.start("Menu");
          self.registerElement.setVisible(true);
        }
        if (event.target.id === "Rstart") {
          self.startOverworldScene();
        }
            ...
            Firebase.auth()
              ...
              .then(function (r) {
                ...
                let posX =
                  ...
                  24 * (self.displayName.length / 2 + 8);
  ...
  preload() {
    ...
    Firebase.auth().onAuthStateChanged(function (user) {
      ...
      if (user !== null) {
        ...
        self.headerText.setText("Welcome " + self.displayName);
        let posX =
          self.cameras.main.centerX - 24 * (self.displayName.length / 2 + 8);
        if (posX < 1) {
          posX = 1;
        }
        self.headerText.setPosition(posX, 25);
  ...
  create() {
    let self = this;
    let userInfoButton = this.add
      .image(25, 25, "usericon")
      .setScale(0.2)
      .setInteractive();
    userInfoButton.setTintFill(0xffffff);
    userInfoButton.on("pointerdown", this.showUserInfo, this);
    this.userInfoElement = this.add
      .dom(this.cameras.main.centerX, this.cameras.main.centerY)
      .createFromCache("userinfo");
    this.userInfoElement.setVisible(false);

    if (this.user == null) {
      this.registerElement = this.add
        .dom(this.cameras.main.centerX, this.cameras.main.centerY)
        .createFromCache("register");
